...
selection through 1-D test case results in Chapter 5.
...
When applying the hybrid method, $V_1$ must be large enough to provide accurate transport
...
All source code for the hybrid $S_N$-diffusion model can be found in my Moltres GitHub fork
repository\footnote{\url{https://github.com/smpark7/moltres/tree/sn-method}} as of the time of
writing. This branch may be merged into the original upstream Moltres GitHub repository in future.
...
$\Psi_{g,d}$ variables by energy group into $G$ \textit{array variables}. An array variable is a
...
employ the \gls{PJFNK} method with the HYPRE-BoomerAMG method for preconditioning. BoomerAMG is an
algebraic multigrid method from the HYPRE \cite{hypre_hypre_2022} linear solver library.

The drift correction terms $\vec{D}_g$ are initialized as auxiliary array variables
...
post-calculation stage from $\Psi_{g,d}$.
...
estimate. This process continues until the neutron flux distribution converges to within tolerance
