...
\end{gather}
%
where $\Psi^\text{inc}_{g,d}=0$ for a vacuum boundary and positive for a boundary source.
...
\section{Transport Correction Formulation}
...
Three main options exist in the literature for applying
transport corrections to the neutron diffusion equations: applying
diffusion coefficient corrections, adding a drift term, or adding a
correction term.

\subsection{Diffusion Coefficient Correction}

Diffusion coefficient corrections involve replacing the diffusion coefficient $D_g$ in the
diffusion term with ``optimal'' diffusion coefficients based on
pointwise transport corrections. The literature review in Chapter \ref{chap:lit} covers two such
...
For the hybrid $S_N$-diffusion method, I used a formulation that incorporates pointwise
...
coefficient variable that varies in space from point to point. We assume that it is continuous and
at least once differentiable except at dissimilar material interfaces. \glspl{SVDC} provide
pointwise corrections to closely match the diffusion flux
...
diffusion coefficient corrections applied through Eq.\ \ref{eq:saaf} were
found to be effective in enabling the neutron diffusion method to accurately reproduce reference
flux solutions \cite{tomatis_application_2011, gross_high-accuracy_2020, tomatis_ronen_2021,
gross_comprehensive_2023, cufe_study_2024, pounders_diffusion_2009}.
However, a significant challenge for space-dependent diffusion coefficients
...
Tomatis \& Dall'Osso
\cite{tomatis_application_2011} developed an alternative formulation in the form of drift terms
while 
