...
between neutron flux and temperature in \glspl{MSR} necessitates robust coupling
...
and substituting the 2-D fuel channel with a 3-D fuel channel that more closely
...
hypothesis that states that the Reynolds stresses in the \gls{RANS} equations
are given by:
%
\begin{align}
...
free shear and backward-facing step flows.
...
Research interest in methods for improving control rod modeling in \glspl{MSR} is subdued compared
to the focus on modeling other physical interactions in \glspl{MSR}.
...
\cite{terrapower_terrapower_2021}. However, \gls{MSR} power control schemes that rely heavily on
...
entering the absorber region. $\lambda$ is derived
...
approximations in ignoring higher-order terms in intermediate steps. Absorber blackness methods
emerged in the mid-20th century when computational resources were limited. Later, as computational
...
The diffusion coefficient formula for this analytic calculation depends on the solution
...
incompatible with reactor geometries that contain control rods that are too close to each other
...
\begin{align}
  ...
  \shortintertext{where}
    ...
    J_D =& \mbox{ diffusion-derived neutron current,} \nonumber \\
...
simulations. Equivalence techniques that reduce spatial homogenization error are also effective
...
\gls{GET} requires diffusion solvers that allow for flux discontinuities at the interfaces.
...
Lastly, there is a lack of existing work into robust techniques for accurate control rod modeling
for time-dependent \gls{MSR} analyses. Control rods induce highly anisotropic fluxes within
themselves and their vicinities that the neutron diffusion approximation cannot model accurately.
Inaccurate rod worth estimates subsequently affect transient reactor power and temperature
distribution calculations. Higher-fidelity neutron transport methods remain too computationally
expensive for time-dependent multiphysics \gls{MSR} simulations. However, transport-correction
